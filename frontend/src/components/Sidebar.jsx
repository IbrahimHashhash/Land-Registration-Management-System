...
const NAV = [
  ...
  {
    label: 'Registrar Review', to: '/review',
    icon: <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><polyline points="9 14 11 16 15 12"/></svg>,
  },
  {
...
export default function Sidebar() {
  ...
  const isAppsActive = location.pathname === '/applications' || location.pathname.startsWith('/applications/')
    ...
    <aside className="w-[236px] shrink-0 bg-[#1a1f1d] flex flex-col px-3.5 py-[18px] sticky top-0 h-screen overflow-y-auto">
      ...
      <nav className="flex-1">
        {NAV.map(({ label, to, icon }) => {
          const active = to === '/applications' ? isAppsActive : location.pathname === to
          return (
            <Link
              key={to}
              to={to}
